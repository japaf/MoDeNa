...
def tessellate(wdir, filename, number_of_cells, visualize_tessellation):
    """Use Laguerre tessellation from Neper to create dry foam. Uses Project01.rco
    as input file."""
    myfile12 = os.path.join(wdir, 'Project01.rco')
    ...
    myfile3 = os.path.join(wdir, 'centers.txt')
        ...
        for i in range(0, 1):
            for j in range(0, number_of_cells):
                fout.write('{0:f}\t{1:f}\t{2:f}\n'.format(centers[j][i],
                                                          centers[j][i + 1],
                                                          centers[j][i + 2]))
    myfile4 = os.path.join(wdir, 'Rads.txt')
    with open(myfile4, 'w') as fout:
        for i in range(0, 1):
            for j in range(0, number_of_cells):
                fout.write('{0:f}\n'.format(rads[j]))
    # Note: Neper regularization is not available for periodic tessellations
    command_tessellation = [
        'neper',
        '-T',
        '-n', '{0:d}'.format(number_of_cells),
        '-domain', 'cube({0:d},{1:d},{2:d})'.format(edge_rve_size,
                                                    edge_rve_size,
                                                    edge_rve_size),
        '-periodicity', 'x,y,z',
        '-morpho', 'voronoi',
    ...
    call = sp.Popen(command_tessellation, cwd=wdir)
    call.wait()
    if visualize_tessellation:
        print('WARNING: vizualization of tessellation currently not implemented.')
    # TODO: update to NEPER 3
    ...
    sdat = read_geo(os.path.join(wdir, filename + ".geo"))
    edat = extract_data(sdat)
    point = edat["point"]
    line = edat["line"]
    with open(os.path.join(wdir, '{0}.gnu'.format(filename)), 'w') as flp:
        for pidx in line.itervalues():
            flp.write('{0} {1} {2}\n'.format(
                point[pidx[0]][0], point[pidx[0]][1], point[pidx[0]][2]))
            flp.write('{0} {1} {2}\n\n\n'.format(
                point[pidx[1]][0], point[pidx[1]][1], point[pidx[1]][2]))
    shutil.copy(os.path.join(wdir, filename + '.geo'),
                os.path.join(wdir, filename + 'Tessellated.geo'))
